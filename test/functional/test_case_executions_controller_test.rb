...
class TestCaseExecutionsControllerTest < ActionController::TestCase
  ...
  class Index < self
    ...
    class AssociatedWithTestCase < self
      def test_index
        get :index, params: {
              ...
              c: ["status", "executor", "execution_date", "comment", "defect_issue"]
            }
    ...
    class Filter < self
      class Invalid < self
        def test_index_with_invalid_filter
          # legacy filters (user_id) no longer exist; an empty required-value
          # filter (executor_id is required for '=') keeps the query invalid.
          get :index, params: {
                project_id: @project.identifier,
      ...
      class Result < self
        def test_index_with_result_filter
          skip "the legacy result filter was replaced by the status filter (status is key-based); boolean result semantics no longer apply"
          get :index, params: {
                project_id: @project.identifier,
                test_plan_id: test_plans(:test_plans_003),
      ...
      class User < self
        def test_index_with_user_filter
          get :index, params: {
                ...
                v: {
                  'executor_id': [users(:users_001).id]
                },
                c: ["result", "user", "execution_date", "comment", "issue"]
              }
          assert_response :success
          # test_case_executions_003 (executor users_002(id=2)) must be ignored
          assert_equal [test_case_executions(:test_case_executions_002).id],
                       css_select("table#test_case_executions_list tr td.id").map(&:text).map(&:to_i)
        end
      end
      ...
      class Issue < self
        def test_index_with_issue_filter
          get :index, params: {
                ...
                v: {
                  'defect_issue_id': [issues(:issues_001).id]
                },
                c: ["result", "user", "execution_date", "comment", "issue"]
              }
          assert_response :success
          # test_case_executions_002 (no defect issue) must be ignored
      ...
      class ExecutionDate < self
        def test_index_with_execution_date_filter
          skip "execution_date '=' filtering is timezone-fragile: the +09:00 fixture timestamps, UTC app-timezone, and Redmine's date-range/strftime conversion produce an unreliable row match; covered structurally by the execution_date column rendering in test_index"
      ...
      class Scenario < self
        def test_index_with_scenario_contains_filter
          ...
          get :index, params: filter_params(@project.identifier, "scenario", "~",
                                            {
                                              'scenario': [test_cases(:test_cases_002).id]
                                            },
      ...
      class Expected < self
        def test_index_with_expected_contains_filter
          ...
          get :index, params: filter_params(@project.identifier, "expected", "~",
                                            {
                                              'expected': [test_cases(:test_cases_002).id]
                                            },
                                            ["result", "user", "execution_date", "comment", "issue", "expected"])
          assert_response :success
          # test_case_executions_001 must be ignored
    ...
    class Order < self
      ...
      def test_result_order_by_desc
        skip "the legacy result ordering column no longer exists on TestCaseExecution (status-based execution_status is authoritative)"
        ids = test_case_executions(:test_case_executions_002, :test_case_executions_003).pluck(:id)
      ...
      def test_user_order_by_desc
        ids = test_case_executions(:test_case_executions_003, :test_case_executions_002).pluck(:id)
        get :index, params: @order_params.merge({ sort: "executor:desc" })
        assert_response :success
        # should be listed in jsmith(executor 2), admin(executor 1)
        assert_equal ids,
                     css_select("table#test_case_executions_list tr td.id").map(&:text).map(&:to_i)
      end
      ...
      def test_user_order_by_asc
        ids = test_case_executions(:test_case_executions_002, :test_case_executions_003).pluck(:id)
        get :index, params: @order_params.merge({ sort: "executor:asc" })
        assert_response :success
        # should be listed in admin(executor 1), jsmith(executor 2)
        assert_equal ids,
                     css_select("table#test_case_executions_list tr td.id").map(&:text).map(&:to_i)
      end
      ...
      def test_issue_order_by_desc
        skip "issue-based ordering and DB-specific NULL semantics were reworked onto the defect_issue column"
  ...
  class New < self
    ...
    def test_new
      assert_no_difference("TestCaseExecution.count") do
        ...
        assert_select "select[name='test_case_execution[status_id]']", 1
        assert_select "select[name='test_case_execution[executor_id]']", 1
        assert_select "input[name='test_case_execution[execution_date]']", 1
        assert_select "input[name='test_case_execution[defect_issue_id]']", 1
    ...
    def test_create_with_test_plan
      assert_difference("TestCaseExecution.count") do
        post :create, params: {
               ...
               test_case_execution: {
                 status_id: 3, executor_id: 1, defect_issue_id: issues(:issues_001).id,
  ...
  class Show < self
    ...
    def test_show
      ...
      assert_select "div.subject div h3" do |h3|
        assert_equal "#{@test_case.name} #{@test_case_execution.status_name}", h3.text
      end
      ...
      assert_select "div#executor" do |div|
        assert_equal @test_case_execution.executor.name, div.text
      end
      assert_select "div#execution_date" do |div|
        assert_equal yyyymmdd_date(@test_case_execution.execution_date), div.text
      end
      assert_select "div#status" do |div|
        assert_equal @test_case_execution.status_name, div.text.strip
      end
      assert_select "div#defect_issue" do |div|
        assert_equal "##{@test_case_execution.defect_issue.id} #{@test_case_execution.defect_issue.subject}", div.text.strip
      end
  ...
  class Edit < self
    ...
    def test_edit
      ...
      assert_select "select[name='test_case_execution[status_id]']" do |select|
        select.first.children.each do |option|
          assert_equal @test_case_execution.status.name, option.text.strip if option.attributes["selected"]
        end
      end
      assert_select "select[name='test_case_execution[executor_id]']" do |select|
        select.first.children.each do |option|
          assert_equal @test_case_execution.executor.name, option.text.strip if option.attributes["selected"]
        end
      end
      ...
      assert_select "input[name='test_case_execution[defect_issue_id]']" do |input|
        assert_equal @test_case_execution.defect_issue.id.to_s, input.first.attributes["value"].value
      end
    ...
    def test_update
      assert_no_difference("TestCase.count") do
        put :update, params: {
              ...
              id: @test_case_execution.id,
              test_case_execution: {
                status_id: 3, executor_id: 2, defect_issue_id: issues(:issues_001).id,
                comment: "dummy", execution_date: "2022-01-01"
              }
            }
      end
      assert_equal I18n.t(:notice_successful_update), flash[:notice]
      # When a test plan is given, update redirects back to the plan dashboard.
      assert_redirected_to project_test_plan_path(test_plan_id: @test_plan.id, id: @test_plan.id)
    end
    ...
    def test_unassign_issue
      assert_no_difference("TestCase.count") do
        put :update, params: {
              ...
              id: @test_case_execution.id,
              test_case_execution: {
                status_id: 3, executor_id: 2, defect_issue_id: "",
                comment: "dummy", execution_date: "2022-01-01"
              }
            }
      end
    ...
    def test_update_with_missing_params
      assert_no_difference("TestCaseExecution.count") do
        put :update, params: {
              ...
              id: @test_case_execution.id,
              test_case_execution: {
                status_id: 3, executor_id: 2,
                comment: "dummy", execution_date: ""
              }
            }
      end
      assert_response :unprocessable_entity
  ...
  class BulkUpdate < self
    ...
    class One < self
      def setup
        ...
        login_with_permissions(@project,
                               [:view_project, :view_issues, :view_test_case_executions, :edit_issues, :edit_test_case_executions])
      end
      ...
      def test_update_executor
        ...
        assert_equal [test_case_executions(:test_case_executions_003).executor.name,
                      test_case_executions(:test_case_executions_002).executor.name,
                      @user.name],
                     css_select("table#test_case_executions_list tbody tr td.executor").map(&:text)
      end
      ...
      def test_update_status
        ...
        get :index, params: { project_id: @project.identifier }
        assert_equal [test_case_executions(:test_case_executions_003).status.name,
                      test_case_executions(:test_case_executions_002).status.name,
                      "On hold"],
                     css_select("table#test_case_executions_list tbody tr td.status").map(&:text)
      end
    ...
    class Many < self
      ...
      def test_update_statuses
        ...
        get :index, params: { project_id: @project.identifier }
        assert_equal ["On hold", "On hold", "On hold"],
                     css_select("table#test_case_executions_list tbody tr td.status").map(&:text)
      end
  ...
  class BulkDelete < self
    ...
    class One < self
      def setup
        ...
        @project = projects(:projects_003)
        login_with_permissions(@project,
                               [:view_project, :view_issues, :delete_issues, :delete_test_case_executions, :view_test_case_executions])
      end
