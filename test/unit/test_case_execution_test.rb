...
class TestCaseExecutionTest < ActiveSupport::TestCase
  ...
  def test_initialize
    ...
    assert_nil test_case_execution.executor_id
    assert_nil test_case_execution.defect_issue_id
    assert_nil test_case_execution.execution_date
    assert_nil test_case_execution.comment
    assert_nil test_case_execution.status
  end
  ...
  def test_create
    ...
    test_case_execution = TestCaseExecution.new(:id => 100,
                                                ...
                                                :test_plan => test_plan,
                                                :test_case => test_case,
                                                :defect_issue => issues(:issues_001))
  ...
  def test_fixture
    ...
    assert_equal users(:users_002), test_case_execution.executor
    assert_equal issues(:issues_001), test_case_execution.defect_issue
    assert_equal test_plans(:test_plans_003), test_case_execution.test_plan
    assert_equal TestExecutionStatus.canonical("passed"), test_case_execution.status
  end
  ...
  def test_missing_status
    ...
    assert_equal [nil, true],
                 [object.status, object.invalid?]
    assert_equal ["cannot be blank"], object.errors[:status]
  end

  def test_missing_executor
    object = TestCaseExecution.new(:status => TestExecutionStatus.canonical("passed"),
                                   :comment => "dummy",
                                   :project => projects(:projects_001),
                                   :test_case => TestCase.new,
                                   :test_plan => TestPlan.new)
    assert_equal true, object.invalid?
    assert_equal ["cannot be blank"], object.errors[:executor]
  end
  ...
  def test_empty_association
    test_case_execution = TestCaseExecution.new
    assert_nil test_case_execution.executor
    assert_nil test_case_execution.project
    assert_nil test_case_execution.defect_issue
    assert_nil test_case_execution.execution_date
  ...
  def test_visible_scope_for_anonymous
    skip "Obsolete: test-content visibility/ownership is now gated by plugin permissions, not inherited from issue permissions."
  ...
  def test_test_case_execution_should_be_readonly_on_closed_project
    test_case_execution = TestCaseExecution.find(1)
    user = User.find(1)
    # The plugin module must be enabled for the project for visible? to grant
    # access even to admins (Project#allows_to? checks enabled modules).
    test_case_execution.project.enabled_module_names += ["testcase_management"]
