...
class TestCaseTest < ActiveSupport::TestCase
  ...
  def test_test_case_should_be_readonly_on_closed_project
    ...
    test_case.project.enabled_module_names += ["testcase_management"]
  ...
  class LatestExecutionTest < self
    ...
    def test_find_with_latest_result
      ...
      latest_execution_in_plan = generate_test_case_execution({
        status: TestExecutionStatus.canonical("failed"),
      ...
      tc = TestCase.find(@test_case.id)
      assert_equal latest_execution.id, tc.latest_test_case_execution.id
      assert_equal latest_execution_in_plan.id, tc.latest_test_case_execution(@test_plan).id
    end

    def test_count
      skip "TestCase.with_latest_result class scope was removed in the status-model refactor; latest-result is now an instance API (#latest_test_case_execution/#latest_status)."
      generate_test_case_execution({
        status: TestExecutionStatus.canonical("passed"),
        execution_date: "2022-04-20",
        test_case: @test_case,
    ...
    def test_across_test_plans
      ...
      tc = TestCase.find(@test_case.id)
      latest = tc.latest_test_case_execution
      assert_equal([true, Date.parse("2022-04-20"), execution.id],
                   [latest.status.passed?, latest.execution_date.to_date, latest.id])
      ...
      tc = TestCase.find(@test_case.id)
      latest = tc.latest_test_case_execution
      assert_equal([false, Date.parse("2022-04-21"), execution.id],
                   [latest.status.passed?, latest.execution_date.to_date, latest.id])
    end
    ...
    def test_with_test_plan
      ...
      latest = test_case.latest_test_case_execution(@test_plan)
      assert_equal([true, Date.parse("2022-04-20"), execution.id],
                   [latest.status.passed?, latest.execution_date.to_date, latest.id])
    end
  end
