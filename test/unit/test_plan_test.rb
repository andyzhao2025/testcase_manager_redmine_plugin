...
class TestPlanTest < ActiveSupport::TestCase
  ...
  fixtures :test_plans, :test_cases, :test_plan_cases
  ...
  def test_missing_issue_status
    skip "Obsolete: issue_status is a deprecated compat column and is now optional (belongs_to optional: true); required-issue-status semantics were removed."
    object = TestPlan.new(:name => "dummy",
                          :user => users(:users_001))
    assert_equal true, object.invalid?
  ...
  def test_test_plan_should_be_readonly_on_closed_project
    ...
    test_plan.project.enabled_module_names += ["testcase_management"]
