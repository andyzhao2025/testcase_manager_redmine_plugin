...
class TestCaseExecutionImportTest < ActiveSupport::TestCase
  ...
  def test_empty_result_error
    ...
    # The empty result maps to no status; the new status model rejects it with a
    # presence validation instead of the old boolean "result" mapping error.
    assert_equal ["Status cannot be blank"],
