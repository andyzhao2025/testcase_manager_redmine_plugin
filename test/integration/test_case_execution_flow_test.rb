...
class TestCaseExecutionFlowTest < Redmine::IntegrationTest
  ...
  test "update test case execution" do
    ...
    put url, params: {
          ...
          test_case_execution: {
            status_id: TestExecutionStatus.canonical('on_hold').id,
            executor_id: 1,
            comment: "dummy",
            execution_date: "2022-01-01"
          }
        }
    # The controller redirects to the plan (run container) when the execution is
    # edited within a test_plan context.
    assert_redirected_to controller: "test_plans",
                         action: "show",
                         test_plan_id: @test_plan.id,
                         id: @test_plan.id
  ...
  def create_test_case_execution(params={})
    post_params = {
      ...
      test_case_execution: {
        executor_id: 1,
        status_id: TestExecutionStatus.canonical('passed').id,
        comment: "dummy",
        execution_date: "2022-01-01"
      }
    }
