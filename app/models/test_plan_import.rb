class TestPlanImport < Import
  AUTO_MAPPABLE_FIELDS = {
    ...
    "plan_state" => "field_plan_state",
    "source_plan" => "field_source_plan",
  ...
  def build_object(row, item)
    ...
    if plan_state = row_value(row, "plan_state")
      attributes["plan_state"] = plan_state if TestPlan::PLAN_STATES.include?(plan_state.to_s)
    end

    if source_plan = row_value(row, "source_plan")
      found_source = if TestPlan.where(name: source_plan, project_id: test_plan.project_id).first
                       TestPlan.where(name: source_plan, project_id: test_plan.project_id).first
                     elsif TestPlan.where(id: source_plan, project_id: test_plan.project_id).first
                       TestPlan.where(id: source_plan, project_id: test_plan.project_id).first
                     end
      attributes["source_plan_id"] = found_source.id if found_source
    end
